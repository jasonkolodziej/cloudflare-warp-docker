...
# Validate runtime dependencies early so failures are actionable.
for cmd in sudo dbus-daemon warp-svc warp-cli gost nft ip jq; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
        echo "[entrypoint] Required command not found: $cmd"
        exit 1
    fi
done
...
if [ ! -f /var/lib/cloudflare-warp/reg.json ]; then
    # Setup-wizard flow for Mesh nodes: use the token from dashboard install commands.
